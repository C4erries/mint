...
// CommandDLQPublisher publishes poison RTC commands into dedicated DLQ Kafka topic.
...
func (p *CommandDLQPublisher) Publish(ctx context.Context, message *commandconsumer.DeadLetterMessage) error {
	if message == nil {
		return fmt.Errorf("command dead letter message is required")
	}

	payload, err := json.Marshal(message)
	if err != nil {
		return fmt.Errorf("marshal command dead letter message: %w", err)
	}
	...
	if err = p.producer.Publish(ctx, p.topic, key, payload); err != nil {
		return fmt.Errorf("publish command dead letter message: %w", err)
	}
