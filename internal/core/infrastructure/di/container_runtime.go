...
// Run starts runtime loops and blocks until context cancellation or first fatal runtime error.
func (c *Container) Run(ctx context.Context) error {
	...
	go func() {
		if runErr := c.outboxRelay.Run(ctx, c.outboxPollInterval); runErr != nil {
			errCh <- fmt.Errorf("run workspace outbox relay: %w", runErr)
		}
	}()

	go func() {
		if runErr := c.workspaceConsumer.Run(ctx); runErr != nil {
			errCh <- fmt.Errorf("run workspace command consumer: %w", runErr)
		}
	}()
