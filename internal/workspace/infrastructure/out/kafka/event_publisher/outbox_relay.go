...
func NewOutboxRelay(store application.OutboxStore, publisher application.EventPublisher, logger *slog.Logger, batchSize int) (*OutboxRelay, error) {
	if store == nil {
	...
	return &OutboxRelay{store: store, publisher: publisher, logger: logger, batchSize: batchSize}, nil
}
	...
	for {
		...
		case <-ticker.C:
			if err := r.FlushOnce(ctx); err != nil {
				r.logger.Error("workspace outbox relay flush failed", slog.String("error", err.Error()))
			}
		}
	}
}
...
func (r *OutboxRelay) FlushOnce(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("list unpublished workspace outbox messages: %w", err)
	}

	for _, message := range messages {
		if err = r.publisher.Publish(ctx, message); err != nil {
			return fmt.Errorf("publish workspace outbox event %s: %w", message.EventID, err)
		}

		if err = r.store.MarkPublished(ctx, message.EventID); err != nil {
			return fmt.Errorf("mark workspace outbox event %s as published: %w", message.EventID, err)
		}
	}

	if len(messages) > 0 {
		r.logger.Info("workspace outbox relay published events", slog.Int("published_events", len(messages)))
	}
