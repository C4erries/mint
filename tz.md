...
- `infrastructure` — transport, adapters, persistence, Kafka, Redis, LiveKit и прочая техническая реализация.
...
Общая схема:
1. Клиент отправляет команду в API.
2. API валидирует аутентификацию, базовый формат и минимум preconditions.
3. API публикует команду в Kafka.
...
Общая схема:
1. Клиент делает query-запрос.
2. Query handler читает уже готовую read model / projection напрямую.
3. Ответ возвращается без Kafka в запросном пути.
